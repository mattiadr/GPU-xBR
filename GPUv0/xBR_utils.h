...
__device__ PixelYUV get_YUV(unsigned int rows, unsigned int cols, PixelYUV *input, unsigned int row, unsigned int col) {
	if (row >= rows) return { 0, 0, 0 }; // TODO fix this
	if (col >= cols) return { 0, 0, 0 };
	return input[row * cols + col];
}

__device__ PixelRGB get_RGB(unsigned int rows, unsigned int cols, PixelRGB *input, unsigned int row, unsigned int col) {
 ...
 **/
__device__ void expand_pixel(unsigned int rows, unsigned int cols, PixelRGB *inputRGB, PixelYUV *inputYUV, PixelRGB *output, unsigned int scaleFactor, unsigned int row, unsigned int col) {
	int edgeBR;
	int edgeBL;
	int edgeTL;
	int edgeTR;
